...
@Composable
fun UpdateToDo(todoWithTags: DtoToDoWithTags, dismiss:()->Unit,update:(todoWithTags: DtoToDoWithTags)->Unit){
    val context = LocalContext.current
    val titleText = remember { mutableStateOf(todoWithTags.todo.title) }
    val descriptionText = remember { mutableStateOf(todoWithTags.todo.description) }
    val selectedDate = remember { mutableStateOf(SimpleDateFormat("dd MMMM yyyy", Locale.getDefault()).format(Date().time)) }
    val selectedDateMillis = remember { mutableLongStateOf(todoWithTags.todo.date) }
    val openDatePicker = remember { mutableStateOf(false) }
    val selectedDueDate = remember { mutableStateOf("") }
    val selectedDueDateMillis = remember { mutableLongStateOf(todoWithTags.todo.dueDate ?: 0L) }
    val openDueDatePicker = remember { mutableStateOf(false) }
    val priorityNumber = remember { mutableIntStateOf(todoWithTags.todo.priority) }
    val openAddTag = remember { mutableStateOf(false) }
    val tags = remember { mutableStateOf(todoWithTags.tags.map { it.name }) }
    ...
    if (openDatePicker.value) {
        DatePickerDialog(
            ...
            val datePickerState = rememberDatePickerState(initialSelectedDateMillis = selectedDateMillis.longValue)
    ...
    if (openDueDatePicker.value) {
        DatePickerDialog(
            ...
            val datePickerState = rememberDatePickerState(initialSelectedDateMillis = selectedDueDateMillis.longValue)
            DatePicker(state = datePickerState)
    ...
    Column(
        ...
        Row(
            ...
            ComponentTransparentButton(stringResource(R.string.update)) {
